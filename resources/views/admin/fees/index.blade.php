                        ...
                        <div class="p-6 text-gray-900">
                            ...
                            <form action="{{ route('admin.fees.store') }}" method="POST">
                                ...
                                <div class="mb-4">
                                     ...
                                     <x-text-input id="type" name="type" class="block mt-1 w-full" placeholder="e.g. Annual, Tuition" required />
                                </div>
                                <div class="mb-4">
                                     <x-input-label for="amount" value="Amount" />
                                     <x-text-input id="amount" type="number" name="amount" class="block mt-1 w-full" step="0.01" required />
                                </div>
                                <div class="mb-4">
                                     <x-input-label for="due_date" value="Due Date" />
                                     <x-text-input id="due_date" type="date" name="due_date" class="block mt-1 w-full" required />
                                </div>
                                <div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($fees as $fee)
                                            <tr>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">{{ $fee->school_class->name }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $fee->type }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold">${{ number_format($fee->amount, 2) }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $fee->due_date->format('d M Y') }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                    <form action="{{ route('admin.fees.destroy', $fee) }}" method="POST" onsubmit="return confirm('Delete this fee structure?');">
                                                        @csrf
                                                        @method('DELETE')
