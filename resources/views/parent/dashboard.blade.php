            ...
            @else
                ...
                <div class="mb-6">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <h3 class="text-lg font-bold text-gray-900">Notice Board</h3>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @forelse($notices as $notice)
                            <div class="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md transition-shadow">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <span
                                        class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ $notice->publish_date->format('d M') }}</span>
                                </div>
                        ...
                        @empty
                            <div
                                ...
                                <p class="text-gray-500 italic text-sm">No new notices for you.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
                                        ...
                                        <thead class="bg-gray-50">
                                            <tr>
                                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">
                                                    Subject</th>
                                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Exam
                                                </th>
                                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">
                                                    Score</th>
                                            </tr>
                                            ...
                                            @foreach($recentMarks as $mark)
                                                <tr>
                                                    ...
                                                    <td class="px-4 py-3 text-sm font-bold text-gray-900">
                                                        {{ $mark->marks_obtained }} / {{ $mark->total_marks }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                ...
                                <div class="space-y-3">
                                    @foreach($recentAttendance as $record)
                                        <div class="flex items-center justify-between border-b pb-2 last:border-0">
                                            <span
                                                class="text-sm font-medium text-gray-700">{{ $record->date->format('d M, Y') }}</span>
